...
const App = () => {
  ...
  const getStatusMessage = (score) => {
    if (score >= 5) return { 
      expertLabel: 'CRITICAL', generalLabel: '心身の限界・要休息',
      color: 'bg-red-600', text: 'white' 
    };
    if (score >= 3.5) return { 
      expertLabel: 'WARNING', generalLabel: '強いストレス警告',
      color: 'bg-orange-500', text: 'white' 
    };
    if (score >= 2) return { 
      expertLabel: 'CAUTION', generalLabel: 'やや不安定',
      color: 'bg-yellow-400', text: 'black' 
    };
    return { 
      expertLabel: 'STABLE', generalLabel: '安定・良好',
  ...
  const getCommunicationStrategy = () => {
    ...
    if (vB >= 2) {
      if (currentMember.bType === 'B2') {
        strategy.push(isExpertMode 
          ? "B2(外向き評価)かつvB高：他者からの承認を必要とします。チームへの貢献を可視化して伝えてください。"
          : "周囲からの評価を気にするタイプです。周囲への良い影響や感謝を具体的に伝えてください。"
        );
      } else {
        strategy.push(isExpertMode 
          ? "高い基準(B)を持っているため、論理的な妥当性と『なぜやるか』の明確化を重視してください。"
          : "強いこだわりを持っています。仕事の目的やルールの必要性を論理的に説明すると納得しやすいです。"
        );
      }
    }
    ...
    if (vA >= 2) {
      strategy.push(isExpertMode 
        ...
        : "モチベーションが高く自走できますが、突然燃え尽きるリスクがあります。定期的にブレーキをかけて休ませてください。"
      );
    }
    ...
    if (currentMember.cType === 'C1') {
      strategy.push(isExpertMode 
        ...
        : "「全体像・仕組み」で理解するタイプです。タスクの背景や全体での位置づけを最初に説明してください。"
      );
    } else if (currentMember.cType === 'C2') {
      strategy.push(isExpertMode 
        ? "C2(場面整序型): メタ認知が構造に寄りすぎています。感情面への配慮を言語化して伝えると効果的です。"
        : "客観的で進行ベースな反面、感情面への配慮が漏れがちです。感情面も「一つの事実」として言語化して伝えてください。"
      );
    } else if (currentMember.cType === 'C3') {
      strategy.push(isExpertMode 
        ? "C3(監視-自己意識型): 監視認知が強いため、細かすぎるマイクロマネジメントは避けてください。"
        : "「人からどう見られているか」に敏感です。過度な監視は避け、ある程度任せる姿勢を見せてください。"
      );
    } else if (currentMember.cType === 'C4') {
      strategy.push(isExpertMode 
        ? "C4(社会文脈型): 社会通念や常識を重んじます。世間的な正しさや社会的意義を強調してください。"
        : "「世間的に正しいか」を重んじます。その仕事の大義名分を伝えるとモチベーションに繋がります。"
      );
    }
    ...
    if (ruinScore > 4) {
      strategy.push(isExpertMode 
        ...
        : "【緊急】非常に高いプレッシャーを感じています。新しいタスクを止め、休息を取れるように調整してください。"
      );
    }
    ...
    if (strategy.length === 0) {
      strategy.push(isExpertMode 
        ...
        : "現在、バランスは安定しています。通常通りのコミュニケーションで問題ありません。"
      );
    }
    ...
    <>
        ...
        <nav className="space-y-1">
          {members.map(m => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-3 md:py-2 rounded-lg text-sm transition-colors ${
                selectedMemberId === m.id ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-600 hover:bg-slate-50'
              }`}
            >
              <div className={`w-2 h-2 rounded-full shrink-0 ${selectedMemberId === m.id ? 'bg-indigo-500' : 'bg-slate-300'}`} />
      ...
      <div className="p-4 border-t border-slate-100">
        <button 
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-3 md:py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-colors"
        >
          <Settings2 size={18} />
          アプリ設定
        </button>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex text-slate-800 font-sans relative">
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-xl">
                <div className="pr-4">
                  ...
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                    ABCモデルの生パラメータ(v, Δ)や、RuinScore等の専門用語を表示します。
                  </p>
                </div>
                <button 
      ...
      {/* Sidebar (Responsive) */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-white border-r border-slate-200 flex flex-col transform transition-transform duration-300 ease-in-out md:relative md:w-64 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden min-w-0 bg-slate-50">
        ...
        {/* Header (Responsive) */}
        <header className="bg-white border-b border-slate-200 px-4 py-3 md:px-8 md:py-4 flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 flex-1">
              <input 
                ...
                placeholder="名前を入力..."
                className="text-lg md:text-xl font-bold bg-transparent border-none outline-none focus:ring-2 focus:ring-indigo-100 rounded px-1 w-full sm:w-48 hover:bg-slate-50 transition-colors placeholder-slate-300"
              />
              <input 
                type="text"
                ...
                placeholder="役職を入力..."
                className="text-xs md:text-sm font-normal text-slate-600 bg-slate-100 px-3 py-1 rounded-full outline-none border border-transparent focus:border-indigo-300 focus:bg-white w-full sm:w-40 hover:bg-slate-200 transition-colors"
              />
            </div>
          </div>
          
          <div className="flex items-center justify-end gap-2 md:gap-3 shrink-0 self-end md:self-auto">
             <div className={`${status.color} text-${status.text} px-3 py-1 md:px-4 md:py-1.5 rounded-full text-[10px] md:text-xs font-bold shadow-sm transition-all whitespace-nowrap`}>
               {isExpertMode ? status.expertLabel : status.generalLabel}
             </div>
             {members.length > 1 && (
               <button 
                 onClick={handleDeleteMember}
                 className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-full transition-colors ml-1"
                 title="メンバーを削除"
               >
                 <Trash2 size={18} />
               </button>
        ...
        {/* Tab Navigation */}
        <div className="bg-white px-4 md:px-8 flex gap-4 md:gap-8 border-b border-slate-200 shrink-0 overflow-x-auto whitespace-nowrap hide-scrollbar">
          {[
            ...
            <button
              ...
              className={`py-3 md:py-4 text-sm font-medium flex items-center gap-2 border-b-2 transition-colors px-1 ${
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
          {activeTab === 'analysis' && (
            <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
              ...
              <div className="lg:col-span-2 space-y-4 md:space-y-6">
                <section className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
                  ...
                  <div className="space-y-4 md:space-y-6">
                    {Object.entries(ABC_TYPES).map(([key, info]) => {
                      ...
                      return (
                        <div key={key} className={`p-4 md:p-5 rounded-xl border-l-4 ${info.border} ${info.bg}`}>
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-2">
                            <label className={`text-sm md:text-base font-bold ${info.color}`}>
                              {isExpertMode ? info.expertLabel : info.generalLabel}
                            </label>
                            {isExpertMode && (
                              <span className="self-start sm:self-auto text-xs font-mono bg-white px-2 py-1 rounded border border-slate-100">
                                v{vVal} / Δ{dVal}
                              </span>
                            )}
                          </div>
                ...
                <section className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
                  <h3 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                    ...
                    {isExpertMode ? 'サブタイプ設定' : '性格のサブタイプ設定'}
                  </h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      >
                        ...
                        <option value="C3">C3: 監視-自己意識型 (他者の目重視)</option>
              ...
              <div className="space-y-4 md:space-y-6">
                ...
                {/* スコア表示領域 */}
                <div className="bg-indigo-900 text-white p-5 md:p-6 rounded-xl shadow-lg relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-4 opacity-10">
                    <ShieldAlert size={80} />
                  </div>
                  <h3 className="text-xs md:text-sm font-bold opacity-80 mb-2">
                    {isExpertMode ? 'RuinScore' : '総合ストレス・リスク値'}
                  </h3>
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl md:text-5xl font-black">{ruinScore}</span>
                    <span className="text-xs md:text-sm opacity-60">/ 6.0</span>
                  </div>
                ...
                {/* 外部要因 (EF) */}
                <section className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
                  <h3 className="text-sm font-bold text-slate-700 mb-4 md:mb-5 flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs leading-relaxed mt-1">
                        {ruinScore > 3 
                          ? (isExpertMode 
                              ? "BがCを凌駕し始めています。Ruin現象への移行を注視してください。" 
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  ...
                  <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div className="space-y-3 md:space-y-4 text-xs md:text-sm">
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-slate-50 rounded-lg gap-2">
                        <span className="text-slate-600 font-medium">{isExpertMode ? 'Cサブタイプ' : '状況把握のクセ'}</span>
                        <span className="font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded sm:text-right">
                          {getCTypeLabel(currentMember.cType, isExpertMode)}
                        </span>
                      </div>
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-slate-50 rounded-lg gap-2">
                        <span className="text-slate-600 font-medium">{isExpertMode ? 'B評価軸' : '基準の置き方'}</span>
                        <span className="font-bold text-amber-600 bg-amber-50 px-3 py-1 rounded sm:text-right">
                          {getBTypeLabel(currentMember.bType, isExpertMode)}
                        </span>
                      </div>
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-slate-50 rounded-lg gap-2">
                        <span className="text-slate-600 font-medium">対人相性推定</span>
                        <span className="font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded sm:text-right">良好（お互いを補える関係）</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <ul className="space-y-3">
                      {[
                        ...
                        <li key={i} className="text-xs md:text-sm flex gap-2 md:gap-3 text-slate-700 leading-relaxed">
               ...
               <section className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200">
                  ...
                  <div className="space-y-3 md:space-y-4">
                    {getCommunicationStrategy().map((tip, i) => (
                      <div key={i} className="flex gap-3 md:gap-4 p-3 md:p-4 border border-slate-100 rounded-xl bg-slate-50/50">
                        <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold shrink-0 text-xs md:text-sm mt-0.5 md:mt-0">
                          {i + 1}
                        </div>
                        <p className="text-xs md:text-sm text-slate-700 leading-relaxed pt-1 md:pt-1">{tip}</p>
                      </div>
                    ))}
                  </div>

                  {/* 話法テンプレート */}
                  <div className="mt-6 md:mt-8 p-4 md:p-6 bg-slate-800 text-slate-100 rounded-xl shadow-inner">
                    <h4 className="text-xs md:text-sm font-bold text-indigo-300 mb-4 md:mb-5 flex items-center gap-2">
                      💡 面談や1on1で使える話法テンプレート
                    </h4>
                    <div className="space-y-5 md:space-y-6 text-xs md:text-sm">
                      <div className="pl-4 border-l-2 border-slate-600 relative">
                        ...
                        <p className="text-slate-400 mb-1.5 md:mb-2 font-medium">
                          ステップ1: {isExpertMode ? 'A（体感）の承認' : 'まずは行動と熱意を認める'}
                        </p>
                        <p className="font-medium text-white leading-relaxed">「最近のプロジェクトでの粘り強さ、非常に頼もしく見ています。」</p>
                      </div>
                      <div className="pl-4 border-l-2 border-slate-600 relative">
                        <div className="absolute -left-2 top-0 w-3.5 h-3.5 rounded-full bg-slate-600 border-4 border-slate-800"></div>
                        <p className="text-slate-400 mb-1.5 md:mb-2 font-medium">
                        ...
                        <p className="font-medium text-white leading-relaxed">「このタスクの品質について、あなた自身が目指しているラインを教えてください。」</p>
                      </div>
                      <div className="pl-4 border-l-2 border-slate-600 relative">
                        <div className="absolute -left-2 top-0 w-3.5 h-3.5 rounded-full bg-slate-600 border-4 border-slate-800"></div>
                        <p className="text-slate-400 mb-1.5 md:mb-2 font-medium">
                        ...
                        <p className="font-medium text-white leading-relaxed">「少し俯瞰してチーム全体のリソース状況を見ると、今は完璧を目指すより8割の出力で進めるのが『構造的に正解』かもしれません。」</p>
                      </div>
                    </div>
                  </div>
               </section>
